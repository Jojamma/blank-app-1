...
if not st.session_state.logged_in:
    ...
    if st.session_state.is_admin:
        # Admin view: Display active users
        st.title("Admin Dashboard")
        st.write("### Active Users")
        
        # Display number of logged-in users
        num_users = len(st.session_state.active_users)
        st.write(f"**Number of Users Logged In:** {num_users}")
        
        # Create a clickable list of usernames with timestamps
        for user in st.session_state.active_users:
            if st.button(user['username']):
                # Show logs for the selected user
                user_logs = [log for log in st.session_state.log_data if log['Username'] == user['username']]
                if user_logs:
                    log_df = pd.DataFrame(user_logs)
                    st.write(f"### Logs for {user['username']}")
                    st.dataframe(log_df)
                else:
                    st.write(f"No logs available for {user['username']}.")
